"""LangGraph compile helpers for confidence-gated agent loops."""
...
class EvalLoopState(TypedDict, total=False):
    project: str
    task: str
    topic: str
...
Proposer = Callable[[EvalLoopState], str]


def run_eval_loop_graph(
    *,
    cfg: dict[str, object],
    ...
    """Compile and run the standard fetch → propose → evaluate → gate loop."""

    def _fetch_source(state: EvalLoopState) -> EvalLoopState:
    ...
    def _propose(state: EvalLoopState) -> EvalLoopState:
    ...
    def _evaluate(state: EvalLoopState) -> EvalLoopState:
    ...
    def _assess(state: EvalLoopState) -> EvalLoopState:
        iteration = int(state.get("iteration", 1))
    ...
    def _route(state: EvalLoopState) -> str:
        return "retry" if bool(state.get("continue_loop")) else "finalize"

    def _retry(state: EvalLoopState) -> EvalLoopState:
    ...
    out: EvalLoopState = app.invoke(initial)
    ...
    result: dict[str, object] = {
        ...
        "answer": out["answer"],
        "trace_events": out.get("trace_events", []),
    }
